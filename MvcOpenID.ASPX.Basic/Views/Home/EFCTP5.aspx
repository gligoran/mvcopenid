...
<h1>MvcOpenID with Entity Framework 4 CTP5</h1>

<p><strong><em>Last updated on: 12/15/2010</em></strong></p>

<h2>Why Entity Framework CTP5?</h2>

<p>Microsoft ADO.NET Entity Framework 4.0 was released as part of .NET 4.0 on 12 April 2010. The CTP5 version is the latest (released 12/6/2010) preview version of Entity Framework 4.0. It is a preview of the Code First Programming Model and Productivity Improvements for Entity Framework 4. The final release of Code First is expected in the first quarter of next year (Q1 of 2011).</p>

<h2>Installing EFCTP5</h2>

<p>There are 2 main ways of including EFCTP5:</p>

<ul>
<li>Download and install <a href="http://www.microsoft.com/downloads/en/details.aspx?FamilyID=35adb688-f8a7-4d28-86b1-b6235385389d">Microsoft ADO.NET Entity Framework Feature Community Technology Preview 5</a>. After the installation is complete add a reference to Microsoft.Data.Entity.CTP.</li>
...
<p>The build still failes at this point. The reason for this is that EF4.0 and EFCTP5 do not use the same base class for UserContext. As UserRepository is using the UserContext the build fails there. So you'll have to modify it a bit.</p>

<p>Open the UserRepository.cs file. In the <em>AddOpenId</em> function you'll see two lines of code just before the end. One is commented out and the other isn't. They both have a comment at the end to let you know to which EF version they apply to. Comment out the EF4 line and uncomment the EFCTP5 line. You see a similar situation in the <em>RemoveOpenId</em> and <em>RemoveUser</em> functions. Do the same there.</p>

<h2>Optional Steps</h2>

<h3>Setting the Database Initializer</h3>

<p>If you never ran any of the MvcOpenID projects using EFCTP5 this (probably) won't affect you. The problem will occur when you change the model. This will break the project at runtime and throw the following exception:</p>
...
<p>To fix this we must allow EFCTP5 to rebuild out database model. You'll achieve this by opening the Global.asax.cs file located at the root of the project. At the top you'll see a commented <code>using</code> statement. Uncomment it. A bit lower, at the top of the <code>Application_Start()</code> function you see another line of code commented out:</p>

<p><code>//Database.SetInitializer&lt;UserContext&gt;(new DropCreateDatabaseIfModelChanges&lt;UserContext&gt;());</code></p>
...
<p>There is a connection string inside web.config that is setup by the Entity Framework for the model. This is no longer needed when using EFCTP5 as you can remove it by going to Web.config file at deleting the following connection string:</p>
